...
void ni2b_init(int max, int max_n, BIN_ARR **st_ptr){
	int i;
	*st_ptr = 
		(BIN_ARR *) malloc (max * sizeof(BIN_ARR));
	for(i=0; i<max; i++){
		(*st_ptr)[i].n = 0;
		(*st_ptr)[i].v = 
			(int *) malloc (max_n * sizeof(int)); 
	}
}
int ni2b_init_file(char *file, int max, BIN_ARR **st_ptr){
	FILE *fp;
...
//		printf("xxxx\n");
	for(i=0; i<max; i++){
		(*st_ptr)[i].v = 
			(int *) malloc ( (*st_ptr)[i].n * sizeof(int) ); 
		(*st_ptr)[i].n = 0;
	}
	return 0;
}
int ni2b_init_file_reverse(char *file, int max, BIN_ARR **st_ptr){
	FILE *fp;
	char line[MAX_LINE_SIZE];
	COUNT line_num;
	char *token;
...
void ni2b_insert(int i, int j, BIN_ARR **st_ptr)
{
	int n=(*st_ptr)[i].n++;
