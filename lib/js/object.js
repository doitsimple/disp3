...
module.exports.setByKey = setByKey;
function setByKey(obj, key, set){
	var rtn = obj;
	var keys = key.split(".");
	...
	for (var key in config){
		if(typeof config[key] == "object"){
			if(isArray(config[key])){
				...
				if(!config2[key] || typeof config2[key] != "object") config2[key] = {};				
...
function forBySeq(obj, fn){
	var tmp = sortByKey(obj, function(a, b){
		...
		return parseInt(obj[a].seq) - parseInt(obj[b].seq);
	});
	for(var key in tmp){
		fn(key, obj[key]);
	}	
